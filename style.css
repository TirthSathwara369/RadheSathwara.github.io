...
function renderPapers() {
    ...
    papersGrid.innerHTML = researchPapers.map(paper => `
        <div class="paper-card" data-id="${paper.id}">
            ${paper.image ? `
                <div class="card-image" onclick="viewImage('${paper.image}', '${paper.title}')">
                    <img src="${paper.image}" alt="${paper.title}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 5px; margin-bottom: 1rem; cursor: pointer;">
                </div>
            ` : ''}
...
function renderThoughts() {
    ...
    thoughtsGrid.innerHTML = scientificThoughts.map(thought => `
        <div class="thought-card" data-id="${thought.id}">
            ${thought.image ? `
                <div class="card-image" onclick="viewImage('${thought.image}', '${thought.title}')">
                    <img src="${thought.image}" alt="${thought.title}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 5px; margin-bottom: 1rem; cursor: pointer;">
                </div>
            ` : ''}
...
function viewPaper(paperId) {
    ...
    // Create a modal to view the file
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <div class="modal-body">
                ${paper.fileType === 'application/pdf' ? 
                    `<iframe src="${paper.fileData}" style="width: 100%; height: 600px; border: none;"></iframe>` :
                    `<div class="file-preview">
                        <p>📄 ${paper.fileName}</p>
                        <p>File type: ${paper.fileType}</p>
                        <p>Size: ${paper.fileSize}</p>
                        <button class="btn btn-primary" onclick="downloadPaper(${paperId})">Download to View</button>
                    </div>`
                }
            </div>
        </div>
...
function closeModal() {
    const modals = document.querySelectorAll('.file-modal');
    modals.forEach(modal => modal.remove());
}

// Data will be loaded from localStorage and persist across page refreshes
